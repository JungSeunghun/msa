package com.pageday.readinghistoryserver.common.jwt.util
...
class JwtUtil(private val jwtProperties: JwtProperties) {
    ...
    fun getUserIdFromAccessToken(accessToken: String) : Long {
        return getClaimsFromToken(accessToken, accessSecretKey).get("userId", Number::class.java).toLong()
    }

    fun getUserIdFromRefreshToken(refreshToken: String) : Long {
        return getClaimsFromToken(refreshToken, refreshSecretKey).get("userId", Number::class.java).toLong()
    }
